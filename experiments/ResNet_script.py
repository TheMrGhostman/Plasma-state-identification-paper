...
from utils.my_blocks import ResNetBlock1d_stride
from utils.layers import  ResNetBlock1d
...
#4) model
f = nn.Sequential(
    nn.Conv1d(xdim[0],options.channels,kernel_size=5, stride=2),
    ResNetBlock1d(in_channels=options.channels, out_channels=options.channels, kernel_size=3, layers=2, activation=nn.ReLU),
    ResNetBlock1d_stride(in_channels=options.channels, out_channels=options.channels*2, kernel_size=3, layers=2, stride=2, activation=nn.ReLU),
    ResNetBlock1d(in_channels=options.channels*2, out_channels=options.channels*2, kernel_size=3, layers=2, activation=nn.ReLU),
    ResNetBlock1d_stride(in_channels=options.channels*2, out_channels=options.channels*4, kernel_size=3, layers=2, stride=2, activation=nn.ReLU),
    ResNetBlock1d(in_channels=options.channels*4, out_channels=options.channels*4, kernel_size=3, layers=2, activation=nn.ReLU),
    ResNetBlock1d(in_channels=options.channels*4, out_channels=options.channels*4, kernel_size=3, layers=2, activation=nn.ReLU),
    ResNetBlock1d_stride(in_channels=options.channels*4, out_channels=options.channels*8, kernel_size=3, layers=2, stride=2, activation=nn.ReLU),
    ResNetBlock1d(in_channels=options.channels*8, out_channels=options.channels*8, kernel_size=3, layers=2, activation=nn.ReLU),
    nn.AdaptiveAvgPool1d(1),
    nn.Flatten(),
    nn.Linear(options.channels*8,4)
    )
...
#7) model name
model_name = f"ResNet_lr={options.lr}_bs={options.batch_size}_scheduler={options.scheduler}_ui={options.ui}"
